...
function PullOutVehicle()
    ...
    elseif ownsVan == false then
        ...
        QBCore.Functions.SpawnVehicle('faggio', function(veh)
...
function DrawTarget()
    ...
    if DeliveriesCount == 4 then
      ...
      if pizza == 1 then
        QBCore.Functions.Notify("You still have one more delivery to go")
      else
        if pizza == 4 then
          pizza = 'Four'
        elseif pizza == 3 then
...
Citizen.CreateThread(function()
    while true do
        ...
        if dist <= 20.0 and Hired and (not HasPizza) and xxx ~= nil then
            local DeliveryPoint = {
                ["x"] = xxx,
                ["y"] = yyy,
                ["z"] = zzz
            ...
            if IsVehicleModel(Vehicle, GetHashKey('faggio')) then
...
function EndOfWork()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if IsVehicleModel(Van, GetHashKey('faggio')) then
