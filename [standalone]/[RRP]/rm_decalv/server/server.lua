...
CreateThread(function()
    ...
    registerCallback('decalv:server:getPlayerJob', function(source, cb, requiredJobs)
        local src = source
        local playerJob = editable('getJob', src)
        if not requiredJobs then return cb(true) end
        local hasJob = false
        ...
        if hasJob then cb(true) else cb(false) end
    end)
